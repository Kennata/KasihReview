...
class TMDBclient(val client: HttpClient) {
    ...
    suspend fun getMoviesByName(name: String): Result<MovieSearchResult, NetworkError> {
        val response = try {
            client.get(urlString = "https://api.themoviedb.org/3/search/movie") {
                url {
    ...
    suspend fun getMovieDetailsById(id: Int): Result<MovieDetails, NetworkError> {
        val response = try {
            client.get(urlString = "https://api.themoviedb.org/3/movie/$id") {
                header(HttpHeaders.Accept, "application/json")
                header(HttpHeaders.Authorization, "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJkODc5NmM1NTRlYWQyYTk2OWQwYTY4NTg5ODZiYWMxYSIsIm5iZiI6MTc2MTE0NzEzNi41ODgsInN1YiI6IjY4ZjhmOTAwM2NkYWRiYmI3MDQ4MzBiOCIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.m_ok0d03kfeaZ_5_wHKBAgBxP90sWgqzd1vaidOdycE")
            }
        }catch(e: UnresolvedAddressException) {
        ...
        return when(response.status.value) {
            in 200..299 -> {
                val result = response.body<MovieDetails>()
                Result.Success(result)
            }
            401 -> Result.Error(NetworkError.UNAUTHORIZED)
            409 -> Result.Error(NetworkError.CONFLICT)
    ...
    suspend fun getMoviesByGenres(genres: String): Result<MovieSearchResult, NetworkError> {
        val response = try {
            client.get(urlString = "https://api.themoviedb.org/3/discover/movie") {
                url {
                    parameters.append("with_genres",genres)
                }
                header(HttpHeaders.Accept, "application/json")
                header(HttpHeaders.Authorization, "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJkODc5NmM1NTRlYWQyYTk2OWQwYTY4NTg5ODZiYWMxYSIsIm5iZiI6MTc2MTE0NzEzNi41ODgsInN1YiI6IjY4ZjhmOTAwM2NkYWRiYmI3MDQ4MzBiOCIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.m_ok0d03kfeaZ_5_wHKBAgBxP90sWgqzd1vaidOdycE")
            }
